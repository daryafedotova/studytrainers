  ...
  return (
    <div className="pointer-events-none fixed inset-0 overflow-hidden" aria-hidden>
      <div
        ...
        style={{
          background: "linear-gradient(135deg, #071827 0%, #0b263c 55%, #123b55 100%)",
        }}
      />

      <motion.div
        className="absolute -left-40 top-[-15%] h-[34rem] w-[34rem] rounded-full bg-cyan-500/18 blur-[120px]"
        animate={{ x: [0, 60, 0], y: [0, 40, 0] }}
        transition={{ duration: 18, repeat: Infinity, ease: "easeInOut" }}
      />
      <motion.div
        className="absolute right-[-12%] top-[28%] h-[30rem] w-[30rem] rounded-full bg-sky-400/12 blur-[120px]"
        animate={{ x: [0, -70, 0], y: [0, 60, 0] }}
        transition={{ duration: 22, repeat: Infinity, ease: "easeInOut" }}
      />
      <motion.div
        className="absolute bottom-[-18%] left-[22%] h-[28rem] w-[28rem] rounded-full bg-violet-500/10 blur-[130px]"
      ...
      <div className="absolute inset-x-0 bottom-0 h-56 bg-gradient-to-t from-[#071827]/80 to-transparent" />
    </div>
  );
}
