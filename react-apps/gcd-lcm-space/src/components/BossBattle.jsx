...
export function BossBattle({profile,onBack,onVictory}) {
  ...
  if(victory)return <section className="boss-end victory-boss"><div className="boss-end-icon"><Sparkles/></div><p className="eyebrow">Ядро нейтрализовано</p><h1>Экспедиция завершена!</h1><p>{profile.displayName}, числовая галактика снова стабильна.</p><div className="boss-end-stats"><span>Жизни: {victory.livesRemaining}/3</span><span>Лучшее комбо: ×{victory.bestCombo}</span></div><div className="button-row"><button className="primary-button" onClick={onBack}>Вернуться на карту</button><button className="secondary-button" onClick={resetBattle}><RotateCcw size={18}/> Сразиться снова</button></div></section>;
