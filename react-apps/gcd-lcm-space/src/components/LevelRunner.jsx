...
import { LevelEnvironment } from './LevelEnvironment.jsx';
...
export function LevelRunner({levelId,onComplete,onBack,onReplay,soundOn=true}) {
  ...
  function advance(nextStats){setFeedback(null);setHintOpen(false);if(index<tasks.length-1){setIndex(i=>i+1);return;}if(phase==='mission'){setPhase('miniboss');setIndex(0);return;}const evaluation=evaluateLevelAttempt(nextStats);setResult(evaluation);setPhase('result');playTone('star',soundOn);if(evaluation.stars===3)confetti({particleCount:90,spread:65,origin:{y:.65},disableForReducedMotion:true});if(!completedRef.current){completedRef.current=true;onComplete(levelId,evaluation);}}
  function submit(answer){if(!task||feedback?.correct)return;const correct=answerMatches(task,answer);playTone(correct?'correct':'wrong',soundOn);const next=registerLevelAnswer(stats,{taskId:task.id,correct,isMiniBoss:phase==='miniboss'});setStats(next);setFeedback({correct});if(correct)window.setTimeout(()=>advance(next),520);}
  const progress=phase==='mission'?(index/mission.length)*100:phase==='miniboss'?100:0;
  if(phase==='briefing')return <section className={`level-screen themed-level level-${levelId}`}><LevelEnvironment levelId={levelId}/><div className="level-ui-layer"><div className="level-header"><button className="ghost-button" onClick={onBack}><ArrowLeft size={18}/> На карту</button><div><span className="mini-label">Миссия {levelId}</span><strong>{level.title}</strong></div></div><BriefingRenderer levelId={levelId} onDone={()=>setPhase('mission')}/></div></section>;
  if(phase==='result')return <section className={`level-screen result-screen themed-level level-${levelId}`}><LevelEnvironment levelId={levelId}/><div className="level-ui-layer"><div className="result-emblem"><Sparkles/></div><p className="eyebrow">Миссия завершена</p><h1>{level.title}</h1><div className="result-stars" aria-label={`${result.stars} звезды`}>{[1,2,3].map(n=><Star key={n} className={n<=result.stars?'filled-star':''} fill={n<=result.stars?'currentColor':'none'}/>)}</div><div className="result-stats"><div><span>С первой попытки</span><strong>{result.firstTryAccuracy}%</strong></div><div><span>Лучшее комбо</span><strong>×{result.bestCombo}</strong></div><div><span>Очки</span><strong>+{result.points}</strong></div></div><div className="button-row"><button className="primary-button" onClick={onBack}>На карту</button><button className="secondary-button" onClick={onReplay}>Пройти ещё раз</button></div></div></section>;
  return <section className={`level-screen themed-level level-${levelId}`}><LevelEnvironment levelId={levelId}/><div className="level-ui-layer"><div className="level-header"><button className="ghost-button" onClick={onBack}><ArrowLeft size={18}/> На карту</button><div className="level-name"><span className="mini-label">{phase==='miniboss'?'Мини-босс':`Миссия ${levelId}`}</span><strong>{level.title}</strong></div><div className="combo-pill"><span>Комбо</span><strong>×{stats.currentCombo}</strong></div></div><div className="level-progress"><div style={{width:`${phase==='miniboss'?100:Math.max(5,progress)}%`}}/></div><motion.article key={task.id} className={`task-card ${phase==='miniboss'?'miniboss-card':''}`} initial={{opacity:0,y:18,scale:.985}} animate={{opacity:1,y:0,scale:1}} transition={{duration:.32,ease:'easeOut'}}><div className="task-kicker">{phase==='miniboss'?`Контроль ${index+1}/${mini.length}`:`Задание ${index+1}/${mission.length}`}</div><h2>{task.prompt}</h2><TaskRenderer task={task} disabled={feedback?.correct} onSubmit={submit}/>{feedback&&<div className={`feedback ${feedback.correct?'correct':'wrong'}`} role="status"><strong>{feedback.correct?'✓ Верно!':'✕ Пока нет'}</strong>{!feedback.correct&&phase!=='miniboss'&&<button className="hint-button" type="button" onClick={openHint}><Lightbulb size={17}/> Подсказка</button>}{!feedback.correct&&phase==='miniboss'&&<span>Попробуй ещё раз без подсказки.</span>}</div>}{hintOpen&&<div className="hint-panel"><BookOpen size={20}/><p>{task.hint}</p></div>}</motion.article></div></section>;
}
